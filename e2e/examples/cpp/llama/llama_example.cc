...
void llama_example(const INIReader reader)
{
   ...
   const bool   sparse               = reader.GetBoolean("ft_instance_hyperparameter", "sparse", false);
   const bool   release_dense        = reader.GetBoolean("ft_instance_hyperparameter", "release_dense", false);
   const int    int8_mode            = reader.GetInteger("ft_instance_hyperparameter", "int8_mode", 0);
   ...
   size_t total_output_len         = reader.GetInteger("request", "total_output_len", 0);
   // The length of tokens we hope this model to generate
   int request_output_len = reader.GetInteger("request", "request_output_len", 0);
   ...
   for (int i = 0; i < request_batch_size; i++) {
    ...
    if (total_output_len > 0 && total_output_len > max_input_len){
        /* code */
        printf("[WARNING] request_output_len change {%d} --> {%d}\n", request_output_len, 
                total_output_len - max_input_len);
        request_output_len = total_output_len - max_input_len;
    } else if (request_output_len > 0){
        total_output_len = max_input_len + request_output_len;
    }else{
        printf("[ERROR] request_total_len > max_input_len or  request_output_len > 0 be must!\n");
        exit(-1);
    }
   ...
   gpt_weights.loadModel(model_dir);
    ...
    if (sparse) {
        ...
        gpt_weights.compress_weights(cublas_wrapper, static_cast<const bool>(release_dense));
    }
#endif
   ...
   for (int i = 0; i < ite; ++i) {
       llama.forward(&output_tensors, &input_tensors, &gpt_weights);
   }
   cudaDeviceSynchronize();
   mpi::barrier();
   POP_RANGE;
   ...
   cudaProfilerStop();
    ...
    char mode_name[6][20] = {"FP16", "W8A16(CUTLASS)", "W8A8(SmoothQuant)", "", "", "W6Ax(FlexQ)"};
   printf("[INFO]Benchmark:%s  request_batch_size %ld beam_width %ld head_num %ld size_per_head %ld total_output_len %d"
