...
#include "src/fastertransformer/models/llamaV2/LlamaV2.h"
...
template<typename T>
void llamaV2_example(const INIReader reader);
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 2) {
        ...
        ini_name = "../examples/cpp/llamaV2/llamaV2_config.ini";
    }
    ...
    if (data_type == "fp32") {
        llamaV2_example<float>(reader);
    }
    else if (data_type == "fp16") {
        llamaV2_example<half>(reader);
    }
#ifdef ENABLE_BF16
    else if (data_type == "bf16") {
        llamaV2_example<__nv_bfloat16>(reader);
    }
#endif
    else {
...
void llamaV2_example(const INIReader reader)
{
    ...
    int debug_mode         = reader.GetInteger("ft_instance_hyperparameter", "debug_mode", 0);
    bool enable_flash_attn = reader.GetBoolean("ft_instance_hyperparameter", "enable_flash_attn", false);

    // TODO: Adapt FlexQ for llama-2.
    if(int8_mode == 5){
        FT_LOG_ERROR("FlexQ for llama-2 is not yet implemented.");
        return ;
    }
    ...
    const float  rope_scaling         = reader.GetFloat(model_name, "rope_scaling", 1.f);
    ...
    if (debug_mode >0 && tensor_para_size > 1) {
        std::this_thread::sleep_for(std::chrono::milliseconds(30000)); // sleep for 30s
    }
    ...
    fastertransformer::LlamaV2Weight<T> gpt_weights(head_num,
                                                    kv_head_num,
                                                    size_per_head,
                                                    inter_size,
                                                    vocab_size,
    ...
    LlamaV2<T> gpt = LlamaV2<T>(head_num,
                                kv_head_num,
                                size_per_head,
                                inter_size,
                                decoder_layers,
...
//                    printf("%d, ", hBuf[i]);
    ...
    char mode_name[7][20] = {"FP16", "W8A16(CUTLASS)", "W8A8(SmoothQuant)", "", "", "", "W6Ax(FlexQ)"};
