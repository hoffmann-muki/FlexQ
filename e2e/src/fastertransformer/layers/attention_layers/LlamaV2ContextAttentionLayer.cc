...
#include "src/fastertransformer/layers/attention_layers/LlamaV2ContextAttentionLayer.h"
...
template<typename T>
void LlamaV2ContextAttentionLayer<T>::forward(TensorMap*                output_tensors,
                                              ...
                                              const AttentionWeight<T>* attention_weights)
{
    ...
    if (use_sparse) {
        ...
        cublas_wrapper_->Gemm(CUBLAS_OP_N,
                              ...
                              attention_input,
                              hidden_units_,  // k
                              enable_flash_attn_ ? qkv_buf_ : qkv_buf_tmp_,
                              local_qkv_size /* n */);
        if (local_kv_head_num_ != local_head_num_ && !enable_flash_attn_) {
            invokeKvRepeat(qkv_buf_, qkv_buf_tmp_, local_head_num_, local_kv_head_num_, size_per_head_, m, stream_);
        }
    }
    ...
    {
        invokeGqaAddFusedQKVBiasTranspose(q_buf_2_,
                                          k_buf_2_,
                                          v_buf_2_,
                                          param,  // prefix prompt
                                          qkv_buf_,
...
//
    ...
    invokeTranspose4dBatchMajor(output_tensors->getPtr<T>("key_cache"),
                                ...
                                max_seq_len,
                                size_per_head_,
                                enable_flash_attn_ ? local_kv_head_num_ : local_head_num_,
                                local_kv_head_num_,
                                stream_);
...
template<typename T>
LlamaV2ContextAttentionLayer<T>::LlamaV2ContextAttentionLayer(size_t           max_batch_size,
                                                              size_t           max_seq_len,
                                                              size_t           head_num,
                                                              size_t           kv_head_num,
...
template<typename T>
LlamaV2ContextAttentionLayer<T>::LlamaV2ContextAttentionLayer(LlamaV2ContextAttentionLayer<T> const& attention_layer):
    ...
    rope_scaling_(attention_layer.rope_scaling_),
...
template<typename T>
LlamaV2ContextAttentionLayer<T>::~LlamaV2ContextAttentionLayer()
{
...
template<typename T>
void LlamaV2ContextAttentionLayer<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void LlamaV2ContextAttentionLayer<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool allocate_qk_buf)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (std::getenv("BYTENN_FLASH_ATTENTION") != nullptr) {
        enable_flash_attn_ = std::string(std::getenv("BYTENN_FLASH_ATTENTION")) == "ON";
...
template<typename T>
void LlamaV2ContextAttentionLayer<T>::freeBuffer()
{
    if (std::getenv("BYTENN_FLASH_ATTENTION") != nullptr) {
        enable_flash_attn_ = std::string(std::getenv("BYTENN_FLASH_ATTENTION")) == "ON";
    }
...
template class LlamaV2ContextAttentionLayer<float>;
template class LlamaV2ContextAttentionLayer<half>;
#ifdef ENABLE_BF16
template class LlamaV2ContextAttentionLayer<__nv_bfloat16>;
#endif
