...
template<typename T>
class LlamaV2DecoderSelfAttentionLayer: public BaseAttentionLayer<T> {
private:
    // buffer handling
    ...
    std::shared_ptr<FLEXQGEMMWrapper>                       flexq_gemm_wrapper_;
...
protected:
    ...
    char*  flexq_gemm_workspace_   = nullptr;
    size_t flexq_gemm_ws_bytes_    = 0;
...
public:
    LlamaV2DecoderSelfAttentionLayer(size_t           max_batch_size,
                                     size_t           head_num,
                                     size_t           kv_head_num,
    ...
    LlamaV2DecoderSelfAttentionLayer(LlamaV2DecoderSelfAttentionLayer<T> const& attention_layer);

    ~LlamaV2DecoderSelfAttentionLayer();
