                                          ...
                                          const AttentionWeight<T>* attention_weights)
{
    ...
    //    const int  moe_lora_id              = input_tensors->getVal<int>("moe_lora_id");
    ...
    half*    decoder_normed_input_s6scale = nullptr;
    if(input_tensors->isExist("decoder_normed_input_s6scale_")){
        decoder_normed_input_s6scale = input_tensors->getPtr<half>("decoder_normed_input_s6scale_");
    }
    
    half*    out_input_s6scale = nullptr;
    if(input_tensors->isExist("out_input_s6scale_")){
        out_input_s6scale = input_tensors->getPtr<half>("out_input_s6scale_");
    }
    ...
    if (std::getenv("BYTENN_FLASH_ATTENTION") != nullptr) {
        enable_flash_attn_ = std::string(std::getenv("BYTENN_FLASH_ATTENTION")) == "ON";
    }
    ...
    if (use_sparse) {
        ...
        flexq_gemm_wrapper_->gemm(m,
                        3 * local_hidden_units_,  // n
                        hidden_units_,            // k
                        reinterpret_cast<const int*>(attention_input),
                        reinterpret_cast<const int*>(attention_weights->query_weight.kernel),
                        nullptr,
                        reinterpret_cast<half*>(qkv_buf_),
                        reinterpret_cast<float*>(decoder_normed_input_s6scale),
        ...
        // FT_LOG_INFO("cublas_wrapper_->Gemm in SelfAttention");
    ...
    // qkv_buf_[m, 3 * local_hidden_units_] -->
    // q_buf_2_:[batch, head_num, seq_len, size_per_head] ...
    // for int8_mode == 2: qkv_buf_ * query_weight.scale_out + query_weight.bias
    // [1, 54, 5120] -> [1, 40, 54, 128]
    PUSH_RANGE("FusedQKVBiasTranspose");
    ...
    if (enable_flash_attn_) {
        ...
        //  (q [B, H, s, D], k [B, kvH, s, D], v [B, kvH, ,s, D])
        Layout layout_q{
            int(local_head_num_ * request_seq_len * size_per_head_), int(size_per_head_), int(request_seq_len * size_per_head_)};
        Layout layout_k{int(local_head_num_ * request_seq_len * size_per_head_),
                        int(size_per_head_),
                        int(request_seq_len * size_per_head_),
                        false,
                        layer_offset,
                        &k_buf_2_};
        Layout layout_v{int(local_head_num_ * request_seq_len * size_per_head_),
                        int(size_per_head_),
                        int(request_seq_len * size_per_head_),
                        false,
                        layer_offset,
                        &v_buf_2_};
        Layout layout_o{
            int(local_head_num_ * request_seq_len * size_per_head_),
            int(local_head_num_ * size_per_head_),
            int(size_per_head_),
            true, //?
        };
        AttentionOp                  flash_attention(request_batch_size, local_head_num_, request_seq_len, request_seq_len, size_per_head_);
        typename AttentionOp::Params attn_params{qkv_buf_3_,
                                                 ...
                                                 nullptr,
                                                 nullptr, //?
                                                 nullptr,//input_tensors->at("input_lengths").getPtr<int>(),
                                                 1, //group_szie
                                                 layout_q,
                                                 layout_k,
                                                 layout_v,
    ...
    PUSH_RANGE("Transpose4dBatchMajor to KVCache");
    ...
    if (attention_type == AttentionType::FUSED_MHA && !enable_flash_attn_) {
        PUSH_RANGE("Context FUSED_MHA");
    ...
    if (is_final == false) {
        ...
        if (attention_type != AttentionType::FUSED_MHA && !enable_flash_attn_) {
            if (is_qk_buf_float_ == true && gemm_data_type != CUDA_R_32F) {
                ...
                param.attention_score    = qk_buf_;  // (batch_size, head_num, q_length, k_length) //(1, h, 54, 54)
                ...
                //                T *in = new T [attention_seq_len_2];
                //                printf("softmax : %d %d %d %d %d %d %d %d %d\n",
                //                       attention_seq_len_2, head_num_, size_per_head_, attention_seq_len_1,
                //                       request_batch_size, request_seq_len, max_seq_len, max_seq_len_,
                //                       local_head_num_);
                //
                //                cudaD2Hcpy(in, qk_buf_  +  attention_seq_len_2, attention_seq_len_2);
                //                printPerLine((half*)in, attention_seq_len_2, "h1 qk_gemm(1) ");
                //                cudaD2Hcpy(in, qk_buf_ + (attention_seq_len_2-1) * attention_seq_len_2,
                //                attention_seq_len_2); printPerLine((half*)in, attention_seq_len_2, "h1 qk_gemm(-1) ");
                //                cudaD2Hcpy(in, qk_buf_ + (head_num_-1) * (attention_seq_len_2) * attention_seq_len_2,
                //                attention_seq_len_2); printPerLine((half*)in, attention_seq_len_2, "h-1) qk_gemm ");
                //                cudaD2Hcpy(in, qk_buf_ + (head_num_-1) * (attention_seq_len_2) * attention_seq_len_2 +
                //                (attention_seq_len_2-1) * attention_seq_len_2, attention_seq_len_2);
                //                printPerLine((half*)in, attention_seq_len_2, "h-1) qk_gemm(-1) ");
                //                exit(0);
            }
            else {
                PUSH_RANGE("Q*K batch gemm");
            ...
            // in: (batch_size, head_num, q_length, k_length] x (batch_size, head_num, q_length, size_per_head]
            // ps: q len = k len
            // out : (batch_size, head_num, q_length, size_per_head]
            ...
            if (padding_offset == nullptr) {
                // transpose (request_batch_size, local_head_num_, attention_seq_len_1, size_per_head_) 
                // to (request_batch_size, attention_seq_len_1, local_head_num_ * size_per_head_)
                // and quant to int8 based attention_output_weight.scale if int8_mode_ == 2
        ...
        if (use_sparse) {
            ...
            if (int8_mode_ == 1 || int8_mode_ == 4) {
                ...
                flexq_gemm_wrapper_->gemm(m,
                                        ...
                                        reinterpret_cast<const half*>(qkv_buf_3_),
                                        reinterpret_cast<const int*>(attention_weights->attention_output_weight.kernel),
                                        nullptr,
                                        reinterpret_cast<half*>(attention_out),
                                        reinterpret_cast<float*>(out_input_s6scale),
                                        attention_weights->attention_output_weight.scale,
                                        attention_weights->attention_output_weight.scale_inter,
                                        attention_weights->attention_output_weight.scale_out,
                                        false,
                                        flexq_gemm_workspace_,
                                        flexq_gemm_ws_bytes_,
                                        stream_);
            }
            else {
                cublas_wrapper_->Gemm(CUBLAS_OP_N,
...
GptContextAttentionLayer<T>::GptContextAttentionLayer(size_t           max_batch_size,
    ...
    flexq_gemm_wrapper_(int8_mode == 5 ? std::make_shared<FLEXQGEMMWrapper>(6, 6, true) : nullptr),
    ...
    enable_flash_attn_(enable_flash_attn)
{
    if (int8_mode == 1){
        ...
        weight_only_int4_fc_runner_ = nullptr;
    } 
    else if(int8_mode == 4){
        weight_only_int8_fc_runner_ = nullptr;
...
GptContextAttentionLayer<T>::GptContextAttentionLayer(GptContextAttentionLayer<T> const& attention_layer):
    ...
    flexq_gemm_wrapper_(attention_layer.flexq_gemm_wrapper_),
    int8_mode_(attention_layer.int8_mode_),
    enable_flash_attn_(attention_layer.enable_flash_attn_)
{
}
...
void GptContextAttentionLayer<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool allocate_qk_buf)
{
    ...
    if (int8_mode_ == 1) {
        ...
        const int max_size  = std::max(hidden_units_, 3 * local_hidden_units_);
        flexq_gemm_ws_bytes_  = 6 * batch_size * seq_len * max_size / (sizeof(char) * 8);
        flexq_gemm_workspace_ = (char*)allocator_->reMalloc(flexq_gemm_workspace_, flexq_gemm_ws_bytes_, false);
    }
...
void GptContextAttentionLayer<T>::freeBuffer()
{
    ...
    if (is_allocate_buffer_) {
        ...
        if (flexq_gemm_workspace_) {
            allocator_->free((void**)(&flexq_gemm_workspace_));
            flexq_gemm_ws_bytes_ = 0;
        }
