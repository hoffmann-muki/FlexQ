...
#include "src/fastertransformer/layers/attention_layers/LlamaV2DecoderSelfAttentionLayer.h"
...
template<typename T>
void LlamaV2DecoderSelfAttentionLayer<T>::allocateBuffer()
{
    FT_CHECK_WITH_INFO(false, "Deprecated. Use `allocateBuffer(size_t batch_size)` instead");
}

template<typename T>
void LlamaV2DecoderSelfAttentionLayer<T>::allocateBuffer(size_t batch_size)
{
    ...
    if (local_kv_head_num_ != local_head_num_) {
        qkv_buf_tmp_ = reinterpret_cast<T*>(
            allocator_->reMalloc(qkv_buf_tmp_, type_size * batch_size * size_per_head_ * local_q_kv_head_num, false));
    }
    else {
    ...
    if (int8_mode_ == 1) {
        ...
        const int max_size  = std::max(d_model_, 3 * local_hidden_units_);
        flexq_gemm_ws_bytes_  = 6 * batch_size * max_size / (sizeof(char) * 8);
...
template<typename T>
void LlamaV2DecoderSelfAttentionLayer<T>::freeBuffer()
{
...
template<typename T>
bool LlamaV2DecoderSelfAttentionLayer<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
LlamaV2DecoderSelfAttentionLayer<T>::LlamaV2DecoderSelfAttentionLayer(size_t           max_batch_size,
                                                                      size_t           head_num,
                                                                      size_t           kv_head_num,
...
LlamaV2DecoderSelfAttentionLayer<T>::LlamaV2DecoderSelfAttentionLayer(size_t           max_batch_size,
                                                                      ...
                                                                      bool             sparse,
                                                                      int              int8_mode):
    LlamaV2DecoderSelfAttentionLayer<T>(max_batch_size,
                                        head_num,
                                        kv_head_num,
                                        size_per_head,
                                        head_num,
...
template<typename T>
LlamaV2DecoderSelfAttentionLayer<T>::LlamaV2DecoderSelfAttentionLayer(
    LlamaV2DecoderSelfAttentionLayer<T> const& attention_layer):
    LlamaV2DecoderSelfAttentionLayer<T>(attention_layer.max_batch_size_,
...
template<typename T>
void LlamaV2DecoderSelfAttentionLayer<T>::forward(TensorMap*                output_tensors,
                                                  ...
                                                  const AttentionWeight<T>* attention_weights)
{
    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1) {
            ...
            flexq_gemm_wrapper_->gemm(batch_size,
            ...
            if (local_kv_head_num_ != local_head_num_) {
                invokeKvRepeat(
                    qkv_buf_, qkv_buf_tmp_, local_head_num_, local_kv_head_num_, size_per_head_, batch_size, stream_);
            }
        }
    }
    sync_check_cuda_error();
    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1) {
            ...
            flexq_gemm_wrapper_->gemm(batch_size,
                                    ...
                                    reinterpret_cast<const half*>(context_buf_),
...
template class LlamaV2DecoderSelfAttentionLayer<float>;
template class LlamaV2DecoderSelfAttentionLayer<half>;
#ifdef ENABLE_BF16
template class LlamaV2DecoderSelfAttentionLayer<__nv_bfloat16>;
#endif
