                                           ...
                                           const AttentionWeight<T>* attention_weights)
{
    ...
    // [m, d_model_] x [d_model_, 3 * local_hidden_units_] --> [m,  3 * local_hidden_units_] without biasAdd
    // for int8_mode = 2, QuantInput * QuantWeight --> QuantOutput without biasAdd
    PUSH_RANGE("qkv_gemm");
    if (use_sparse_gemm) {
#ifdef SPARSITY_ENABLED
    ...
    // [m,  3 * local_hidden_units_] + biasAdd, do attention --> [m, local_hidden_units_]
    // for int8_mode = 2, QuantInput*scale_out + bias, and  do attention, then quant based attention_output_weight.scale
    PUSH_RANGE("fusedQKV_masked_attention");
    ...
    // [m, local_hidden_units_ --> [m, d_model_] without biasAdd, T type
