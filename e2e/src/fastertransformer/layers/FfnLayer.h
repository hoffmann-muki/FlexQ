...
private:
    ...
    std::shared_ptr<FLEXQGEMMWrapper>                 flexq_W6A6_gemm_wrapper_;
    std::shared_ptr<FLEXQGEMMWrapper>                 flexq_W6A8_gemm_wrapper_;
...
//    void allocateBuffer(size_t token_num, int moe_k = 0, bool use_moe = false, bool use_moe_lora = false);

protected:
    T*    inter_buf_        = nullptr;
    T*    inter_lora_buf_   = nullptr;
    T*    inter_buf_2_      = nullptr;  // for gated activation
    T*    moe_gates_buf_    = nullptr;
    T*    lora_mask_buf_    = nullptr;
    ...
    int*   fc2_packed_input_ = nullptr;
    // For simplicity, we uniformly set the bit width for the `fc2_packed_input` buffer size to 8 bits, which handles both the 6-bit and 8-bit cases.
    size_t fc2_packed_input_bits = 8;
    ...
    */
    ...
    // int8_mode_ == 1 for weight quantized only gemm for GPT W8
    // int8_mode_ == 2 for SmoothQuant O3 (per tensor scales)
    // int8_mode_ == 4 for weight quantized only gemm for GPT W4
    int int8_mode_ = 0;
    ...
    void genericLoraMaskActivation(int          m,
                                   T *          lora_input,
                                   int*         lora_mask,
                                   const T*     bias0,
                                   const T*     bias1,
