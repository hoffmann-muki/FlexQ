...
void FfnLayer<T>::forward(TensorMap* output_tensors, TensorMap* input_tensors, const FfnWeight<T>* ffn_weights)
{
    ...
    half* ffn_normed_input_s6scale = nullptr;
    half* down_input_s8scale = nullptr;
    half* down_input_s6scale = nullptr;
    int quant_bits = 16;  // The bit width of FC2 input activations.
    if (input_tensors->isExist("ffn_normed_input_s6scale_")){
        ffn_normed_input_s6scale  = input_tensors->at("ffn_normed_input_s6scale_").getPtr<half>();
    }
    if (input_tensors->isExist("down_input_s8scale_")){
        down_input_s8scale  = input_tensors->at("down_input_s8scale_").getPtr<half>();
        quant_bits = 8;
    }
    if (input_tensors->isExist("down_input_s6scale_")){
        down_input_s6scale  = input_tensors->at("down_input_s6scale_").getPtr<half>();
        quant_bits = 6;
    }
    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1 || int8_mode_ == 4) {
            ...
            // FT_CHECK_WITH_INFO(!use_gated_activation, "use_gated_activation doesn`t support int8_mode=2 now!");
            ...
            if (use_gated_activation){
                cublas_wrapper_->Int8Gemm(inter_size_,
                                      ...
                                      input_tensors->getPtr<int8_t>("ffn_input"),
                                      hidden_units_,
                                      reinterpret_cast<int8_t*>(inter_buf_2_),
                                      inter_size_,
                                      ffn_weights->intermediate_weight2.scale_inter);
            }
        }
        else if (int8_mode_ == 5) {
            flexq_W6A6_gemm_wrapper_->gemm(m,
                                    inter_size_,
                                    hidden_units_,
                                    reinterpret_cast<const int*>(input_tensor),
                                    reinterpret_cast<const int*>(ffn_weights->intermediate_weight.kernel),
                                    nullptr,
                                    reinterpret_cast<half*>(inter_buf_),
                                    reinterpret_cast<float*>(ffn_normed_input_s6scale),
                                    ffn_weights->intermediate_weight.scale,
                                    ffn_weights->intermediate_weight.scale_inter,
                                    ffn_weights->intermediate_weight.scale_out,
                                    false,
                                    nullptr,
                                    0,
                                    stream_);
            ...
            if (use_gated_activation) {
                flexq_W6A6_gemm_wrapper_->gemm(m,
                                        ...
                                        hidden_units_,
                                        reinterpret_cast<const int*>(input_tensor),
                                        reinterpret_cast<const int*>(ffn_weights->intermediate_weight2.kernel),
                                        nullptr,
                                        reinterpret_cast<half*>(inter_buf_2_),
                                        reinterpret_cast<float*>(ffn_normed_input_s6scale),
                                        ffn_weights->intermediate_weight2.scale,
                                        ffn_weights->intermediate_weight2.scale_inter,
                                        ffn_weights->intermediate_weight2.scale_out,
                                        false,
                                        nullptr,
                                        0,
                                        stream_);
    ...
    if ((int8_mode_ != 1 && int8_mode_ != 4 && int8_mode_ != 2) || ia3_tasks != nullptr || use_gated_activation) {
        ...
        genericActivation(m,
                          ...
                          down_input_s8scale == nullptr ? down_input_s6scale : down_input_s8scale,
                          quant_bits,
                          input_tensors->getPtr<int>("padding_offset", nullptr),
                          input_tensors->getVal<int>("seq_len", 1));
        POP_RANGE;
    }
    // TODO: support use_gated_activation and int8_mode == 2
    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1 || int8_mode_ == 4) {
            ...
            if(down_input_s6scale != nullptr){
                flexq_W6A6_gemm_wrapper_->gemm(m,
                    hidden_units_,
                    inter_size_,
                    reinterpret_cast<const int*>(fc2_packed_input_),
                    reinterpret_cast<const int*>(ffn_weights->output_weight.kernel),
                    nullptr,
                    reinterpret_cast<half*>(output_tensor),
                    reinterpret_cast<float*>(down_input_s6scale),
                ...
                /*
                    Adaptation: To address the issue of misaligned context token_nums, we use `inter_buf_2_` to represent the output matrix by allocating a larger memory space.
                */
                flexq_W6A8_gemm_wrapper_->gemm(m,
                        hidden_units_,
                        inter_size_,
                        reinterpret_cast<const int*>(fc2_packed_input_),
                        reinterpret_cast<const int*>(ffn_weights->output_weight.kernel),
                        nullptr,
                        reinterpret_cast<half*>(inter_buf_2_),
                        reinterpret_cast<float*>(down_input_s8scale),
                        ffn_weights->output_weight.scale,
                        ffn_weights->output_weight.scale_inter,
                        ffn_weights->output_weight.scale_out,
                        false,
                        ...
                        0,
                        stream_);
                output_tensor = inter_buf_2_;
            }

            sync_check_cuda_error();
        }
        else {
            // FT_LOG_INFO("hidden_units_ %d, m %d, inter_size_ %d", hidden_units_, m, inter_size_);
            cublas_wrapper_->Gemm(CUBLAS_OP_N,
                                  CUBLAS_OP_N,
                                  hidden_units_,
                                  m,
    ...
    int8_fc_runner_(int8_mode == 2 ? std::make_shared<CutlassInt8GemmRunner<T>>() : nullptr)
{
    ...
    if (int8_mode_ == 0) {
        ...
        FT_CHECK_WITH_INFO(!(std::is_same<T, float>::value), "Weight only quant(W8) not supported for fp32.");
        ...
        moe_int4_weight_only_fc_runner_ = nullptr;
        weight_only_int4_fc_runner_     = nullptr;
    }
    else if (int8_mode_ == 4) {
        FT_CHECK_WITH_INFO(!(std::is_same<T, float>::value), "Weight only quant(W4) not supported for fp32.");
        ...
        moe_int8_weight_only_fc_runner_ = nullptr;
        weight_only_int8_fc_runner_     = nullptr;
    }
    else if (int8_mode_ == 5) {
        // w6a6
        flexq_W6A6_gemm_wrapper_ = std::make_shared<FLEXQGEMMWrapper>(6, 6, true);
        // w6a8
        flexq_W6A8_gemm_wrapper_ = std::make_shared<FLEXQGEMMWrapper>(8, 6, true);
    }
}
...
void FfnLayer<T>::allocateBuffer(size_t token_num, int moe_k, bool use_moe)
{
    ...
    if (use_moe) {
        ...
        if (int8_mode_ == 1 ) {
            ...
            fc2_packed_input_ = (int*)allocator_->reMalloc(fc2_packed_input_, (max_inter_size_ / 128) * token_num * fc2_packed_input_bits * 4 * sizeof(int), false);
        }
    }
...
void FfnLayer<T>::freeBuffer()
{
    ...
    if (is_allocate_buffer_) {
        ...
        if(fc2_packed_input_){
            allocator_->free((void**)(&fc2_packed_input_));
        }
...
#define INVOKE_GENERIC_ACT(ACT)                                                                                        \
    invokeGenericActivation<ACT>(fc2_packed_input_,                                                                    \
                                 inter_buf_,                                                                           \
                                 bias1,                                                                                \
                                 inter_buf_2_,                                                                         \
                                 bias2,                                                                                \
...
template<typename T>
SiluFfnLayer<T>::SiluFfnLayer(SiluFfnLayer<T> const& silu_ffn_layer): FfnLayer<T>(silu_ffn_layer)
{
}
