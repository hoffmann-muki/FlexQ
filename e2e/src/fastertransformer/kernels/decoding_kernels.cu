...
__global__ void embeddingLookupPosEncoding(T*             from_tensor,
                                           ...
                                           const int      local_token_num, //local_batch_size * beam_width
                                           const int64_t  hidden_units,
                                           const int      step, //step_ - 1
                                           const int      max_input_length, // 0
                                           const int      token_num, //batch x beam
                                           const int      ite, // 0
                                           const T        scale) //1.f
{
    ...
    for (int64_t index = blockIdx.x * blockDim.x + threadIdx.x; index < local_token_num * hidden_units;
        ...
        // Adaptation: Use direct zeroing to skip computation for invalid tokens.
        int id_val = 0;
        if(all_ids[id_offset + row_index] > 0){
            id_val = all_ids[id_offset + row_index];
        }
        T val = embedding_table[id_val * hidden_units + col_index] * scale;
...
void invokeEmbeddingLookupPosEncoding(T*                    from_tensor,
                                      ...
                                      const int             step, // step_-1
                                      const int             max_input_length, // 0
                                      const int             token_num,//batch_size * beam_width
                                      const int             ite,     //0
                                      const int             seq_len, //0
                                      cudaStream_t          stream)
{
                                              ...
                                              cudaStream_t          stream)
{
    invokeEmbeddingLookupPosEncoding<T>(from_tensor,
                                        ...
                                        nullptr,
                                        prompt_param,
                                        local_token_num, //local_batch_size * beam_width
                                        hidden_units,
                                        scale, //1.f
                                        step, //step_1 -1
                                        0,
                                        token_num, //batch_size * beam_width
                                        ite, // 0
                                        seq_len, // 0
                                        stream);
}
...
// PROMPT_SRC: 0 --> no prompts, 1 --> from loaded prompts, 2 --> from request prompts
template<typename T>
__global__ void onlyEmbeddingLookupPosEncoding(T*             from_tensor,
                                               const T*       embedding_table,
                                               const T*       position_encoding,
                                               const int*     all_ids,
                                               const int*     padding_count,
...
/* Adapter function for invokeEmbeddingLookupPosEncoding{PadCount,InputLen} */
template<typename T>
void invokeOnlyEmbeddingLookupPosEncoding(T*                    from_tensor,
                                          const T*              embedding_table,
                                          const T*              position_encoding,
                                          const int*            all_ids,
                                          const int*            padding_count,
...
template<typename T>
void invokeOnlyEmbeddingLookupPosEncodingPadCount(T*                    from_tensor,
                                                  const T*              embedding_table,
                                                  const T*              position_encoding,
                                                  const int*            all_ids,
                                                  const int*            pad_count,
                                                  ...
                                                  cudaStream_t          stream)
{
    invokeOnlyEmbeddingLookupPosEncoding<T>(from_tensor,
                                        embedding_table,
                                        position_encoding,
                                        all_ids,
                                        pad_count,
...
#define INSTANTIATE_LOOKUP_POS_ENCODING_PAD_COUNT(T)                                                                   \
    template void invokeOnlyEmbeddingLookupPosEncodingPadCount(T*                    from_tensor,                          \
                                                               const T*              embedding_table,                      \
                                                               const T*              position_encoding,                    \
                                                               const int*            all_ids,                              \
                                                               const int*            pad_count,                            \
