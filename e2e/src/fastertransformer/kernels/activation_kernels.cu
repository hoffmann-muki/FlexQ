...
template<template<typename T> class Activation, typename T, typename BT>
__global__ void flexq_generic_activation(int*              packed_out,
                                   T*                      out,
                                   const BT*  __restrict   bias,
                                   const T*   __restrict   gated_weights,
                                   const BT*  __restrict   gated_bias,
                                   ...
                                   int n)
{
    ...
    const int height = m;
    const int width = n * 2;
    const int bits = quant_bits;
    half* T_in = reinterpret_cast<half*>(out); 
    ...
    const int grid_x_offset = (height + BLOCK_M - 1) / BLOCK_M;
    const int bx = blockIdx.x % grid_x_offset;
    const int by = blockIdx.x / grid_x_offset;
...
#pragma unroll
        ...
        int lower_bound = -(1 << (bits - 1));       // -2 ^ (bits - 1)
        int upper_bound = (1 << (bits - 1)) - 1;    // 2 ^ (bits - 1) - 1
        ...
        if(threadIdx.x % threads_each_group == 0){
            output_scale[scale_offset] = output_scale[scale_offset + 1] = maxv;
        }
    ...
    int output_lane_num = (bits * BLOCK_M * BLOCK_K / BITS_INT4);
    for(int output_lane_id = threadIdx.x; output_lane_id < output_lane_num; output_lane_id += THREADS_NUM){
        ...
        const int bit_T_out_index = (by * (BLOCK_K / BITS_INT4) + output_k) * (height * bits * INT4_NUIT)
                                        + output_chunk_m_id * (bits * chunk_M * INT4_NUIT) + output_bit * (chunk_M * INT4_NUIT) + output_chunk_m_row * INT4_NUIT;

        const int shmem_index = output_bit * offset_shmem_bit + output_m * BLOCK_K / BITS_INT + output_k * INT4_NUIT;

        *(reinterpret_cast<int4 *>(packed_out + bit_T_out_index)) = *((int4 *)(shmem + shmem_index));
    }
}

// clang-format on
template<template<typename T> class Activation, typename T, typename BT>
__global__ void generic_mask_activation(T*                      out0,
                                        T*                      out1,
                                        int*                    mask,
                                        const BT*  __restrict   bias0,
                                        const BT*  __restrict   bias1,
                                        const T*   __restrict   gated_weights,
                                        const BT*  __restrict   gated_bias,
                                        const int* __restrict   ia3_tasks,
                                        const T*   __restrict   ia3_weights,
                                        ...
                                        int n)
{
    ...
    const bool with_bias0 = bias0 != nullptr;
    const bool with_bias1 = bias1 != nullptr;
    const bool with_gate = gated_weights != nullptr;
    const bool with_ia3  = ia3_tasks != nullptr;
    ...
    for (int id = blockIdx.x * blockDim.x + threadIdx.x; id < m * n; id += blockDim.x * gridDim.x) {
        T val[2];
        if (int8_mode == 2) {
            val[0] = cuda_cast<T>(cuda_cast<Float_T>(reinterpret_cast<Packed_Int8_t*>(out0)[id]) * activation_in[0]);
            val[1] = cuda_cast<T>(cuda_cast<Float_T>(reinterpret_cast<Packed_Int8_t*>(out1)[id]) * activation_in[0]);
        }
        else {
            val[0] = out0[id];
            val[1] = out1[id];
        ...
        const T reg_bias0 = static_cast<T>(bias0[id % n]);
        const T reg_bias1 = static_cast<T>(bias1[id % n]);
        val[0]              = (val[0] + (with_bias0 ? static_cast<T>(bias0[id % n]) : val[0]));
        val[1]              = (val[1] + (with_bias1 ? static_cast<T>(bias1[id % n]) : val[1]));
        if (mask[id / n] == 0) {
            val[0] = val[0]; // 1 - mask
        } else {
            val[0] = val[1];
        ...
        if (with_gate) {
            val[0] = cuda_cast<T>(Activation<T>::apply(val[0]) * cuda_cast<Act_T>(gated_val));
        }
        else {
            val[0] = cuda_cast<T>(Activation<T>::apply(val[0]));
        }
        ...
        if (with_ia3) {
            ...
            const int batch_id = (word_id + offset) / seq_len;
            const int task = ia3_tasks[batch_id];
            val[0]            = val[0] * ia3_weights[task * n + (id % n)];
        }
        ...
        if (int8_mode != 2) {
            ...
            reinterpret_cast<Packed_Int8_t*>(out0)[id] =
                cuda_cast<Packed_Int8_t>(cuda_cast<Float_T>(val[0]) * activation_out[0]);
        }
    }
}
// clang-format on
                             ...
                             cudaStream_t stream)
{
    ...
    // flexQ act quant
    bool flexq_act_quant = (output_scale == nullptr ? false : true);

    dim3 block, grid;
    if(flexq_act_quant){
        block.x = THREADS_NUM;
        grid.x  = ceil(m * n / THREADS_NUM);
    }
    else if (n / 4 / packed_elems <= 512) {
    ...
    if(flexq_act_quant){
        const size_t shmem_size = (BLOCK_M * BLOCK_K) * 2 * sizeof(int);
        flexq_generic_activation<Activation><<<grid, block, shmem_size, stream>>>(packed_out,  // int
                                                               reinterpret_cast<PT*>(out),  // half2
                                                               reinterpret_cast<const PBT*>(bias),  // half2
                                                               reinterpret_cast<const PT*>(gated_weights),  // half2
                                                               reinterpret_cast<const PBT*>(gated_bias),  // half2
                                                               ia3_tasks,  // int
                                                               reinterpret_cast<const PT*>(ia3_weights),  // half2
                                                               int8_mode,
                                                               activation_in,  // float
                                                               activation_out,  // float
                                                               output_scale,
                                                               quant_bits,
                                                               padding_offset,
                                                               seq_len,
        ...
        generic_activation<Activation><<<grid, block, 0, stream>>>(reinterpret_cast<PT*>(out),  // half2
                                                            reinterpret_cast<const PBT*>(bias),  // half2
                                                            reinterpret_cast<const PT*>(gated_weights),  // half2
                                                            reinterpret_cast<const PBT*>(gated_bias),  // half2
                                                            ia3_tasks,  // int
...
#define INSTANTIATE_GENERIC_ACTIVATION(Activation, T, BT)                                                              \
    template void invokeGenericActivation<Activation, T, BT>(int*         packed_out,                                  \
                                                             T * out,                                                  \
                                                             const BT*    bias,                                        \
                                                             const T*     gated_weights,                               \
                                                             const BT*    gated_bias,                                  \
                                 ...
                                 cudaStream_t stream)
{
    ...
    generic_mask_activation<Activation><<<grid, block, 0, stream>>>(
        reinterpret_cast<PT*>(out0),
        reinterpret_cast<PT*>(out1),
        (mask),
        reinterpret_cast<const PBT*>(bias0),
        reinterpret_cast<const PBT*>(bias1),
...
#define INSTANTIATE_GENERIC_MASK_ACTIVATION(Activation, T, BT)                                                             \
    template void invokeGenericMaskActivation<Activation, T, BT>(T * out0,                                                 \
                                                                 T * out1,                                                 \
                                                                 int * mask,                                                 \
                                                                 const BT*    bias0,                                        \
                                                                 const BT*    bias1,                                        \
...
INSTANTIATE_GENERIC_MASK_ACTIVATION(GeluActivation, float, float);
INSTANTIATE_GENERIC_MASK_ACTIVATION(GeluActivation, half, half);
#ifdef ENABLE_BF16
INSTANTIATE_GENERIC_MASK_ACTIVATION(GeluActivation, __nv_bfloat16, __nv_bfloat16);
#endif

INSTANTIATE_GENERIC_MASK_ACTIVATION(ReluActivation, float, float);
INSTANTIATE_GENERIC_MASK_ACTIVATION(ReluActivation, half, half);
#ifdef ENABLE_BF16
INSTANTIATE_GENERIC_MASK_ACTIVATION(ReluActivation, __nv_bfloat16, __nv_bfloat16);
#endif

INSTANTIATE_GENERIC_MASK_ACTIVATION(SiluActivation, float, float);
INSTANTIATE_GENERIC_MASK_ACTIVATION(SiluActivation, half, half);
#ifdef ENABLE_BF16
INSTANTIATE_GENERIC_MASK_ACTIVATION(SiluActivation, __nv_bfloat16, __nv_bfloat16);
#endif

INSTANTIATE_GENERIC_MASK_ACTIVATION(IdentityActivation, float, float);
INSTANTIATE_GENERIC_MASK_ACTIVATION(IdentityActivation, half, half);
INSTANTIATE_GENERIC_MASK_ACTIVATION(IdentityActivation, float, half);
#ifdef ENABLE_BF16
INSTANTIATE_GENERIC_MASK_ACTIVATION(IdentityActivation, __nv_bfloat16, __nv_bfloat16);
INSTANTIATE_GENERIC_MASK_ACTIVATION(IdentityActivation, float, __nv_bfloat16);
#endif
#undef INSTANCIATE_GENERIC_MASK_ACTIVATION
...
template<typename T2, int N>
__global__ void loraMaskAddMaskBiasGeluV2(T2* out0,
                                  T2* out1,
                                  int* mask,
                                  const T2* __restrict bias0,
                                  const T2* __restrict bias1,
                                  const int* ia3_tasks,
                                  const T2*  ia3_weights,
                                  const int  size,
                                  const int* padding_offset,
                                  const int  seq_len)
{
    ...
    for (int id = blockIdx.x * blockDim.x + threadIdx.x; id < size; id += blockDim.x * gridDim.x) {
        T2 val0 = out0[id];
        T2 val1 = out1[id];
        T2 reg_bias0 = bias0 ? ldg(&bias0[id % N]) : T2(0, 0);
        T2 reg_bias1 = bias1 ? ldg(&bias1[id % N]) : T2(0, 0);
        if (mask[id / N] == 0) {
            val0 = hadd2(val0, reg_bias0);
        } else {
            val0 = hadd2(val1, reg_bias0);
        }
        val0 = GeluActivation<T2>::apply(val0);
        if (with_ia3) {
            const int word_id  = id / N;
            const int offset   = padding_offset == nullptr ? 0 : padding_offset[word_id];
            const int batch_id = (word_id + offset) / seq_len;
            const int task     = ia3_tasks[batch_id];
            val0                = val0 * ia3_weights[task * N + (id % N)];
        }
        out0[id] = val0;
    }
}
...
template<typename T2, int N, int ELEMENT_PER_ROUND>
__global__ void loraMaskAddBiasGeluV3(T2* out0,
                                      T2* out1,
                                      int* mask,
                                      const T2* __restrict bias0,
                                      const T2* __restrict bias1,
...
{
    //N = half of n = half_n
    //total thread = m * half_n
    const bool with_ia3 = ia3_tasks != nullptr;
    T2         buffer0[ELEMENT_PER_ROUND];
    T2         tmp_bias0[ELEMENT_PER_ROUND];
    T2         buffer1[ELEMENT_PER_ROUND];
    T2         tmp_bias1[ELEMENT_PER_ROUND];
    for (int id = blockIdx.x * blockDim.x * ELEMENT_PER_ROUND + threadIdx.x * ELEMENT_PER_ROUND; id < size;
         id += blockDim.x * gridDim.x * ELEMENT_PER_ROUND) {
#pragma unroll
        for (int i = 0; i < ELEMENT_PER_ROUND; i++) {
            buffer0[i] = out0[id + i];
            buffer1[i] = out1[id + i];
            tmp_bias0[i] = bias0 ? ldg(&bias0[(id + i) % N]) : tmp_bias0[i];
            tmp_bias1[i] = bias1 ? ldg(&bias1[(id + i) % N]) : tmp_bias1[i];
        }
#pragma unroll
        for (int i = 0; i < ELEMENT_PER_ROUND; i++) {
            buffer0[i] = bias0 ? hadd2(buffer0[i], tmp_bias0[i]) : buffer0[i];
            buffer1[i] = bias1 ? hadd2(buffer1[i], tmp_bias1[i]) : buffer1[i];
            if (mask[(id + i) / N] == 0) {
                buffer0[i] = buffer0[i];
            } else {
                buffer0[i] = buffer1[i];
            }
            buffer0[i] = GeluActivation<T2>::apply(buffer0[i]);
            if (with_ia3) {
                const int word_id  = (id + i) / N;
                const int offset   = padding_offset == nullptr ? 0 : padding_offset[word_id];
                const int batch_id = (word_id + offset) / seq_len;
                const int task     = ia3_tasks[batch_id];
                buffer0[i]          = buffer0[i] * ia3_weights[task * N + ((id + i) % N)];
            }
            out0[id + i] = buffer0[i];
        }
    }
...
#define ADD_LORA_MASK_BIAS_GELU(HALF_N, ELEMENT_PER_ROUND)                                                                  \
    case HALF_N:                                                                                                            \
        if (ELEMENT_PER_ROUND > 1) {                                                                                        \
            grid.x = grid.x / ELEMENT_PER_ROUND;                                                                            \
            loraMaskAddBiasGeluV3<T2, HALF_N, ELEMENT_PER_ROUND><<<grid, block, 0, stream>>>(                               \
                (T2*)out0, (T2*)out1, mask, (const T2*)bias0, (const T2*)bias1, ia3_tasks, (T2*)ia3_weights, m * half_n, padding_offset, seq_len);          \
        }                                                                                                                   \
        else {                                                                                                              \
            loraMaskAddMaskBiasGeluV2<T2, HALF_N><<<grid, block, 0, stream>>>(                                                      \
                (T2*)out0, (T2*)out1, mask, (const T2*)bias0, (const T2*)bias1, ia3_tasks, (T2*)ia3_weights, m * half_n, padding_offset, seq_len);          \
        }                                                                                                                   \
        break;
    ...
    if (n % 2 == 0 && sizeof(T) == 2) {
        ...
        if (grid.x >= 512) {
            switch (half_n) {
                ...
                default:
                    invokeGenericActivation<GeluActivation>(nullptr,
                                                            out,
                                                            bias,
                                                            (T*)nullptr,
                                                            (T*)nullptr,
            ...
            switch (half_n) {
                ...
                ADD_BIAS_GELU(1536, 1)
                ADD_BIAS_GELU(2048, 1)
                ADD_BIAS_GELU(4096, 1) //2
                ADD_BIAS_GELU(8192, 2)
                ADD_BIAS_GELU(16384, 2)
                ADD_BIAS_GELU(24576, 2)
                ADD_BIAS_GELU(40960, 2)//4
                default:
                    invokeGenericActivation<GeluActivation>(nullptr,
                                                            out,
                                                            bias,
                                 ...
                                 cudaStream_t stream)
{
    if (n % 2 == 0 && sizeof(T) == 2) {
        ...
        if (grid.x >= 512) {
            switch (half_n) {
                ADD_LORA_MASK_BIAS_GELU(256, 1)
                ADD_LORA_MASK_BIAS_GELU(512, 1)
                ADD_LORA_MASK_BIAS_GELU(1024, 1)
                ADD_LORA_MASK_BIAS_GELU(1536, 1)
                ADD_LORA_MASK_BIAS_GELU(2048, 1)
                ADD_LORA_MASK_BIAS_GELU(4096, 2)
                ADD_LORA_MASK_BIAS_GELU(8192, 2)
                ADD_LORA_MASK_BIAS_GELU(16384, 2)
                ADD_LORA_MASK_BIAS_GELU(24576, 2)
                ADD_LORA_MASK_BIAS_GELU(40960, 4)
                default:
                    invokeGenericMaskActivation<GeluActivation>(out0,
                                                                out1,
                                                                mask,
                                                                bias0,
                                                                bias1,
            ...
            switch (half_n) {
                ...
                ADD_LORA_MASK_BIAS_GELU(1536, 1)
                ADD_LORA_MASK_BIAS_GELU(2048, 1)
                ADD_LORA_MASK_BIAS_GELU(4096, 1)
                ADD_LORA_MASK_BIAS_GELU(8192, 2)
                ADD_LORA_MASK_BIAS_GELU(16384, 2)
                ADD_LORA_MASK_BIAS_GELU(24576, 2)
                ADD_LORA_MASK_BIAS_GELU(40960, 2)
                default:
                    invokeGenericMaskActivation<GeluActivation>(out0,
                                                                out1,
                                                                mask,
...
template void invokeLoraMaskAddBiasGeluV2(float*       out0,
                                      float*       out1,
                                      int*       mask,
                                      const float* bias0,
                                      const float* bias1,
                                      ...
                                      const int    n,
                                      cudaStream_t stream);
template void invokeLoraMaskAddBiasGeluV2(half*        out0,
                                      half*        out1,
                                      int*        mask,
                                      const half*  bias0,
                                      const half*  bias1,
                                      ...
                                      cudaStream_t stream);
#ifdef ENABLE_BF16
template void invokeLoraMaskAddBiasGeluV2(__nv_bfloat16*       out0,
                                      __nv_bfloat16*       out1,
                                      int*       mask,
