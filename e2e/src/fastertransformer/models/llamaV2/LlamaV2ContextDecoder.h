...
template<typename T>
class LlamaV2ContextDecoder: public BaseLayer {
private:
    // meta data
    size_t head_num_;
    size_t kv_head_num_;
...
public:
    LlamaV2ContextDecoder(size_t                              head_num,
                          size_t                              kv_head_num,
    ...
    LlamaV2ContextDecoder(LlamaV2ContextDecoder<T> const& decoder);

    ~LlamaV2ContextDecoder();

    void forward(std::vector<Tensor>*                              output_tensors,
                 const std::vector<Tensor>*                        input_tensors,
                 const std::vector<LlamaV2DecoderLayerWeight<T>*>* decoder_layer_weights);
