...
#include "src/fastertransformer/models/llamaV2/LlamaV2Decoder.h"
#include "src/fastertransformer/layers/TensorParallelSiluFfnLayer.h"
#include "src/fastertransformer/layers/attention_layers/TensorParallelLlamaV2DecoderSelfAttentionLayer.h"

namespace fastertransformer {

template<typename T>
void LlamaV2Decoder<T>::initialize()
{
    self_attention_layer_ = new TensorParallelLlamaV2DecoderSelfAttentionLayer<T>(0,  // max_batch_size
                                                                                  head_num_,
                                                                                  kv_head_num_,
                                                                                  size_per_head_,
...
template<typename T>
void LlamaV2Decoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void LlamaV2Decoder<T>::allocateBuffer(size_t batch_size)
{
...
template<typename T>
void LlamaV2Decoder<T>::freeBuffer()
{
...
template<typename T>
bool LlamaV2Decoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool LlamaV2Decoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool LlamaV2Decoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int LlamaV2Decoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
LlamaV2Decoder<T>::LlamaV2Decoder(size_t                              head_num,
                                  size_t                              kv_head_num,
...
template<typename T>
LlamaV2Decoder<T>::LlamaV2Decoder(LlamaV2Decoder<T> const& decoder):
...
template<typename T>
LlamaV2Decoder<T>::~LlamaV2Decoder()
{
...
template<typename T>
void LlamaV2Decoder<T>::forward(std::vector<Tensor>*                              output_tensors,
...
template<typename T>
void LlamaV2Decoder<T>::forward(std::unordered_map<std::string, Tensor>*          output_tensors,
...
template class LlamaV2Decoder<float>;
template class LlamaV2Decoder<half>;
#ifdef ENABLE_BF16
template class LlamaV2Decoder<__nv_bfloat16>;
#endif
