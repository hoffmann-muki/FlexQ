...
#include "src/fastertransformer/models/llamaV2/LlamaV2ContextDecoder.h"
#include "src/fastertransformer/kernels/bert_preprocess_kernels.h"
#include "src/fastertransformer/kernels/gpt_kernels.h"

#include "src/fastertransformer/layers/TensorParallelSiluFfnLayer.h"
#include "src/fastertransformer/layers/attention_layers/TensorParallelLlamaV2ContextAttentionLayer.h"

namespace fastertransformer {

template<typename T>
void LlamaV2ContextDecoder<T>::initialize()
{
    self_attention_layer_ = new TensorParallelLlamaV2ContextAttentionLayer<T>(0,  // max_batch_size
                                                                              0,  // max_seq_len
                                                                              head_num_,
                                                                              kv_head_num_,
                                                                              size_per_head_,
...
template<typename T>
void LlamaV2ContextDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void LlamaV2ContextDecoder<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool use_shared_contexts)
{
    ...
    if(int8_mode_ == 5){
        decoder_normed_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(decoder_normed_input_s6scale_, sizeof(half) * batch_size * hidden_units_ / group_size, false));
        out_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(out_input_s6scale_, sizeof(half) * batch_size * hidden_units_ / group_size, false));
        ffn_normed_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(ffn_normed_input_s6scale_, sizeof(half) * batch_size * hidden_units_ / group_size, false));
        down_input_s8scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(down_input_s8scale_, sizeof(half) * batch_size * hidden_units_ / group_size, false));
    }
...
template<typename T>
void LlamaV2ContextDecoder<T>::freeBuffer()
{
...
template<typename T>
bool LlamaV2ContextDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool LlamaV2ContextDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool LlamaV2ContextDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int LlamaV2ContextDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
LlamaV2ContextDecoder<T>::LlamaV2ContextDecoder(size_t                              head_num,
                                                size_t                              kv_head_num,
...
template<typename T>
LlamaV2ContextDecoder<T>::LlamaV2ContextDecoder(LlamaV2ContextDecoder<T> const& decoder):
    ...
    rope_scaling_(decoder.rope_scaling_),
...
template<typename T>
LlamaV2ContextDecoder<T>::~LlamaV2ContextDecoder()
{
...
template<typename T>
void LlamaV2ContextDecoder<T>::forward(std::vector<Tensor>*                              output_tensors,
                                       const std::vector<Tensor>*                        input_tensors,
                                       const std::vector<LlamaV2DecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
...
template<typename T>
void LlamaV2ContextDecoder<T>::forward(std::unordered_map<std::string, Tensor>*          output_tensors,
    ...
    for (int ite = 0; ite < iteration_num; ite++) {
        ...
        for (int l = 0; l < num_layer_; l++) {
            ...
            if(int8_mode_ == 5){
                ...
                "decoder_normed_input_s6scale_", Tensor{MEMORY_GPU, data_type, {local_batch_size, hidden_units_ / group_size}, decoder_normed_input_s6scale_});

                self_attention_input_tensors.insert(
                "out_input_s6scale_", Tensor{MEMORY_GPU, data_type, {local_batch_size, hidden_units_ / group_size}, out_input_s6scale_});
            }
            if (is_unpadded_mha) {
            ...
            if (use_shared_contexts) {
                ...
                const size_t cache_stride_per_batch =
                    kv_head_num_ * size_per_head_ / tensor_para_.world_size_ * max_seq_len;
            ...
            if (is_final == false) {
                ...
                TensorMap ffn_input_tensors(
                {
                    ...
                    {"ffn_normed_input_s6scale_", Tensor{MEMORY_GPU, data_type, {local_batch_size, hidden_units_ / group_size}, ffn_normed_input_s6scale_}},
                    {"down_input_s8scale_", Tensor{MEMORY_GPU, data_type, {local_batch_size, hidden_units_ / group_size}, down_input_s8scale_}}
                });
...
template class LlamaV2ContextDecoder<float>;
template class LlamaV2ContextDecoder<half>;
#ifdef ENABLE_BF16
template class LlamaV2ContextDecoder<__nv_bfloat16>;
#endif
