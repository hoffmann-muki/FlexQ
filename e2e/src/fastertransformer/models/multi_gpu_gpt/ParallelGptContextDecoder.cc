...
void ParallelGptContextDecoder<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool use_shared_contexts)
{
    ...
    if(int8_mode_ == 5){
        decoder_normed_packed_input_ = reinterpret_cast<int*>(
            allocator_->reMalloc(decoder_normed_packed_input_, sizeof(int) * (hidden_units_ / 128) * batch_size * seq_len * decoder_input_x_bits * 4, false));
        normed_packed_self_attn_output_ = reinterpret_cast<int*>(
            allocator_->reMalloc(normed_packed_self_attn_output_, sizeof(int) * (hidden_units_ / 128) * batch_size * seq_len * attn_output_x_bits * 4, false));
        decoder_normed_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(decoder_normed_input_s6scale_, 2 * sizeof(half) * batch_size * hidden_units_ / group_size, false));
        out_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(out_input_s6scale_, 2 * sizeof(half) * batch_size * hidden_units_ / group_size, false));
        ffn_normed_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(ffn_normed_input_s6scale_, 2 * sizeof(half) * batch_size * hidden_units_ / group_size, false));
        down_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(down_input_s6scale_, 2 * sizeof(half) * batch_size * inter_size_ / group_size, false));
    }
...
void ParallelGptContextDecoder<T>::freeBuffer()
{
    if (is_allocate_buffer_) {
        ...
        if(int8_mode_ == 5){
            ...
            allocator_->free((void**)(&down_input_s6scale_));
        }
    ...
    const std::vector<ParallelGptDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
    ...
    for (uint ite = 0; ite < iteration_num; ite++) {
        ...
        for (uint l = 0; l < num_layer_; l++) {
            ...
            // FlexQ
            if(int8_mode_ == 5){
                // For adaptation, keep `h_token_num` as input_query.shape[0]
                // self_attention_input_tensors.insert("input_query", Tensor{MEMORY_GPU, activation_in_type, {hidden_units_ / 128, h_token_num * decoder_input_x_bits * 4}, decoder_normed_packed_input_});
                self_attention_input_tensors.insert("input_query", Tensor{MEMORY_GPU, activation_in_type, {h_token_num, hidden_units_ / 128 * decoder_input_x_bits * 4}, decoder_normed_packed_input_});
                self_attention_input_tensors.insert(
                ...
                self_attention_input_tensors.insert("input_query", Tensor{MEMORY_GPU, activation_in_type, {h_token_num, hidden_units_}, layernorm_type_ == LayerNormType::pre_layernorm ? decoder_normed_input_ : decoder_input});
            }
            ...
            TensorMap ffn_input_tensors(
            {
                // {"ffn_input", Tensor{MEMORY_GPU, activation_in_type, {h_token_num, hidden_units_}, layernorm_type_ == LayerNormType::pre_layernorm ? normed_self_attn_output_ : after_adapter_attn_output_}},
                ...
                {"down_input_s6scale_", Tensor{MEMORY_GPU, data_type, {local_batch_size, 2 * inter_size_ / group_size}, down_input_s6scale_}}
            });

            // FlexQ
            if(int8_mode_ == 5){
                // For adaptation, keep `h_token_num` as input_query.shape[0]
                // ffn_input_tensors.insert("ffn_input", Tensor{MEMORY_GPU, activation_in_type, {hidden_units_ / 128, h_token_num * attn_output_x_bits * 4}, normed_packed_self_attn_output_});
                ffn_input_tensors.insert("ffn_input", Tensor{MEMORY_GPU, activation_in_type, {h_token_num, hidden_units_ / 128 * attn_output_x_bits * 4}, normed_packed_self_attn_output_});
            }else{
                ffn_input_tensors.insert("ffn_input", Tensor{MEMORY_GPU, activation_in_type, {h_token_num, hidden_units_}, layernorm_type_ == LayerNormType::pre_layernorm ? normed_self_attn_output_ : after_adapter_attn_output_});
            }
