...
void ParallelGptDecoderLayerWeight<T>::copyFrom(const ParallelGptDecoderLayerWeight& other)
{
    ...
    if (int8_mode_ == 0) {
        ...
        if (int8_mode_ == 1) {
            ...
            cudaD2Dcpy(scale_ptr[0], other.scale_ptr[0], hidden_units_ * (3 * hidden_units_) / group_size / tensor_para_size_);
            cudaD2Dcpy(scale_ptr[1], other.scale_ptr[1], hidden_units_ / tensor_para_size_ * (hidden_units_) / group_size);
            cudaD2Dcpy(scale_ptr[2], other.scale_ptr[2], hidden_units_ * inter_size_ / tensor_para_size_ / group_size);
            cudaD2Dcpy(scale_ptr[3], other.scale_ptr[3], inter_size_ / tensor_para_size_ * hidden_units_ / group_size);
...
void ParallelGptDecoderLayerWeight<T>::mallocWeights()
{
    ...
    if (int8_mode_ == 0) {
        ...
        deviceMalloc(&weights_ptr[8], (w_bits_ * hidden_units_) * (inter_size_ / sz_) / tensor_para_size_);  // ffn inter weight
        deviceMalloc(&weights_ptr[10],  (w_bits_ * inter_size_) / tensor_para_size_ * (hidden_units_ / sz_));  // ffn output weight
    
        // Alloc scale for weight activation quant
        deviceMalloc(&scale_ptr[0], hidden_units_ * (3 * hidden_units_) / group_size / tensor_para_size_);
        deviceMalloc(&scale_ptr[1], hidden_units_ / tensor_para_size_ * (hidden_units_) / group_size);
        deviceMalloc(&scale_ptr[2], hidden_units_ * inter_size_ / tensor_para_size_ / group_size);
        deviceMalloc(&scale_ptr[3], inter_size_ / tensor_para_size_ * hidden_units_ / group_size);
