...
void LlamaDecoder<T>::allocateBuffer(size_t batch_size)
{
   ...
   decoder_layer_output_ = reinterpret_cast<T*>(
    ...
    // FlexQ
    if(int8_mode_ == 5){
        decoder_normed_packed_input_ = reinterpret_cast<int*>(
            allocator_->reMalloc(decoder_normed_packed_input_, sizeof(int) * (hidden_units_ / 128) * batch_size * decoder_input_x_bits * 4, false));
        normed_packed_self_attn_output_ = reinterpret_cast<int*>(
            allocator_->reMalloc(normed_packed_self_attn_output_, sizeof(int) * (hidden_units_ / 128) * batch_size * attn_output_x_bits * 4, false));
        decoder_normed_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(decoder_normed_input_s6scale_, sizeof(half) * 2 * batch_size * hidden_units_ / group_size, false));
        out_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(out_input_s6scale_, sizeof(half) * 2 * batch_size * hidden_units_ / group_size, false));
        ffn_normed_input_s6scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(ffn_normed_input_s6scale_, sizeof(half) * 2 * batch_size * hidden_units_ / group_size, false));
        down_input_s8scale_ = reinterpret_cast<half*>(
            allocator_->reMalloc(down_input_s8scale_, sizeof(half) * 2 * batch_size * inter_size_ / group_size, false));
    }
...
void LlamaDecoder<T>::freeBuffer()
{
   if (is_allocate_buffer_ == true) {
       ...
       // FlexQ
        if(int8_mode_ == 5){
            allocator_->free((void**)(&decoder_normed_packed_input_));
            allocator_->free((void**)(&normed_packed_self_attn_output_));
            allocator_->free((void**)(&decoder_normed_input_s6scale_));
            allocator_->free((void**)(&out_input_s6scale_));
            allocator_->free((void**)(&ffn_normed_input_s6scale_));
            allocator_->free((void**)(&down_input_s8scale_));
        }
                             ...
                             const std::vector<LlamaDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
   ...
   for (uint l = 0; l < num_layer_; l++) {
       ...
       if (isFirstLayerParallelId(l) == true && pipeline_para_.rank_ != 0 && pipeline_para_.world_size_ > 1) {
        ...
        // layer_input:[local_batch_size, hidden_units_] -- > [local_batch_size, hidden_units_]
        // if int8_mode = 2, based on query_weight.scale, quant tensor to int8
        PUSH_RANGE("pre-mha layernorm");
        invokeGeneralLayerNorm(decoder_normed_packed_input_,
       ...
       TensorMap self_attention_input_tensors(*input_tensors);
        ...
        if(int8_mode_ == 5){
            // For adaptation, keep `local_batch_size` as input_query.shape[0]
            // self_attention_input_tensors.insert(
            //     "input_query", Tensor{MEMORY_GPU, data_type, {hidden_units_ / 128, local_batch_size * decoder_input_x_bits * 4}, decoder_normed_packed_input_});
            self_attention_input_tensors.insert(
                "input_query", Tensor{MEMORY_GPU, data_type, {local_batch_size, hidden_units_ / 128 * decoder_input_x_bits * 4}, decoder_normed_packed_input_});
            self_attention_input_tensors.insert(
                "decoder_normed_input_s6scale_", Tensor{MEMORY_GPU, data_type, {local_batch_size, 2 * hidden_units_ / group_size}, decoder_normed_input_s6scale_});
            self_attention_input_tensors.insert(
                "out_input_s6scale_", Tensor{MEMORY_GPU, data_type, {local_batch_size, 2 * hidden_units_ / group_size}, out_input_s6scale_});
        }else{
            self_attention_input_tensors.insert(
       ...
       // [local_batch_size, hidden_units_] --> [local_batch_size, hidden_units_] 
       // output withput biasAdd, for int8_mode=2, output has been dequant to dtype
       PUSH_RANGE("decoder self-attention block");
       self_attention_layer_->forward(&self_attention_output_tensors,
                                      &self_attention_input_tensors,
                                      &layer_weight->self_attention_weights);
       POP_RANGE;

       // AddBiasResidualPreLayerNorm 
       // for int8_mode=2, need quant to int8 based on >ffn_weights.intermediate_weight.scale
    //    FT_CHECK_WITH_INFO(int8_mode_ != 2, "TODO: modify AddBiasResidualPreLayerNorm for Qaunt output!");
       PUSH_RANGE("decoder self-attention AddBiasResidualPreLayerNorm");
       if (use_gptj_residual_) {
           invokeGeneralLayerNorm(decoder_normed_input_,
                                  layer_input,
                                  layer_weight->post_attention_layernorm_weights.gamma,
                                  layer_weight->post_attention_layernorm_weights.beta,
                                  layernorm_eps_,
                                  local_batch_size,
                                  hidden_units_,
                                  (float*)layer_weight->ffn_weights.intermediate_weight.scale,
                                  int8_mode_,
                                  stream_);
       }
       else {
           invokeGeneralAddResidualT5PreLayerNorm(
               self_attn_output_,
               decoder_normed_input_,
               normed_packed_self_attn_output_,
               layer_input,
               layer_weight->post_attention_layernorm_weights.gamma,
               layernorm_eps_,
               local_batch_size,
       ...
       TensorMap ffn_input_tensors(
        {
            ...
            {"ffn_normed_input_s6scale_", Tensor{MEMORY_GPU, data_type, {local_batch_size, 2 * hidden_units_ / group_size}, ffn_normed_input_s6scale_}},
            // For the LLaMA model, the FC2 layer's input activations are kept at a higher bit width (8-bit).
            {"down_input_s8scale_", Tensor{MEMORY_GPU, data_type, {local_batch_size, 2 * inter_size_ / group_size}, down_input_s8scale_}}
        });
        // FlexQ
        if(int8_mode_ == 5){
            // For adaptation, keep `local_batch_size` as input_query.shape[0]
            // ffn_input_tensors.insert("ffn_input", Tensor{MEMORY_GPU, data_type, {hidden_units_ / 128, local_batch_size * attn_output_x_bits * 4}, normed_packed_self_attn_output_});
            ffn_input_tensors.insert("ffn_input", Tensor{MEMORY_GPU, data_type, {local_batch_size, hidden_units_ / 128 * attn_output_x_bits * 4}, normed_packed_self_attn_output_});
        }else{
            ffn_input_tensors.insert("ffn_input", Tensor{MEMORY_GPU, data_type, {local_batch_size, hidden_units_}, decoder_normed_input_});
        }
       ...
       // [local_batch_size, hidden_units_] --> [local_batch_size, hidden_units_] 
       // output withput biasAdd, for int8_mode=2, input is Quantx and Output has been Dequant
       PUSH_RANGE("decoder ffn block");
       ffn_layer_->forward(&ffn_output_tensors, &ffn_input_tensors, &layer_weight->ffn_weights);
       POP_RANGE;

       // TODO: modify for int8_mode=2
       PUSH_RANGE("decoder ffn AddBiasResidual");
       if (use_gptj_residual_) {
           // Original workflow:
