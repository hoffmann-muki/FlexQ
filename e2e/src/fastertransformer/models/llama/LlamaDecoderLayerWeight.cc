...
void LlamaDecoderLayerWeight<T>::copyFrom(const LlamaDecoderLayerWeight& other)
{
    ...
    if (int8_mode_ == 0) {
        ...
        cudaD2Dcpy(scale_ptr[3], other.scale_ptr[3], hidden_units_ * inter_size_ / tensor_para_size_ / group_size);
        cudaD2Dcpy(scale_ptr[4], other.scale_ptr[4], inter_size_ / tensor_para_size_ * hidden_units_ / group_size);
...
void LlamaDecoderLayerWeight<T>::mallocWeights()
{
   ...
   if (int8_mode_ == 0) {
       ...
       deviceMalloc(&weights_ptr[10],  inter_size_ / tensor_para_size_ * hidden_units_);  // output_weight kernel
        ...
        deviceMalloc(&scale_ptr[3], hidden_units_ * inter_size_ / tensor_para_size_ / group_size);
        deviceMalloc(&scale_ptr[4], inter_size_ / tensor_para_size_ * hidden_units_ / group_size);
