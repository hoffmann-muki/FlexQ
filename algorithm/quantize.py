"""Quantization harness with DuQuant support and optional smoothing.

Profiles activations, applies optional smoothing, and quantizes the model
using FlexQ or DuQuant methods.

Usage examples:
python algorithm/quantize.py --model ../models/llama-2-7b-hf --dataset wikitext2 --nsamples 16

This script supports DuQuant rotation-based quantization and optional activation smoothing.
"""
...
def profile_down_proj_layers(lm, dataloader, percentiles=(0.999, 0.9999), max_batches=8, device=None):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            batches_run += 1
            logger.info(f"    Completed batch {batches_run}")
...
def run_sweep(
    ...
    for clip_pct in percentiles_to_try:
        for zero_scale in zero_shifts_to_try:
            ...
            a.adaptive_clip_down_proj = False
            a.adaptive_clip_percentile = 1.0
            a.adaptive_zero_shift_scale = 0.0
        ...
        if isinstance(m, nn.Linear):
            hooks.append(
                m.register_forward_hook(
                    functools.partial(stat_input_hook, name)))
...
def main():
    ...
    parser.add_argument("--output_csv", type=str, default="algorithm/duquant_results.csv")
    ...
    parser.add_argument(
        "--layer_stats_path",
        type=str,
        default="algorithm/duquant_layer_stats.csv",
        help="path to append per-layer stats; leave empty to disable",
    )
    parser.add_argument(
        "--layer_schedule_path",
        type=str,
        default="algorithm/duquant_layer_schedule.csv",
    ...
    if isinstance(getattr(args, "torch_dtype", None), str):
        ...
        if td == "auto":
            ...
            try:
                ...
                logger.warning(f"Unknown torch dtype '{td}', defaulting to torch.float16")
    ...
    if args.duquant:
        logger.info("Running DuQuant...")
        args.deactive_amp = True  # Disable AMP to prevent NaN issues
        lm = LMClass(args)
        lm.seqlen = 2048
        
        # Convert model to float if learning is enabled to avoid dtype mismatches
        if args.epochs > 0:
            lm.model = lm.model.float()
        
        # Profile to find sensitive layers
        profiles, channel_maxes = profile_down_proj_layers(lm, dataloader, percentiles=(0.9999,), max_batches=4)
        key = "p9999"
        top_k_layers = find_top_k_layers(profiles, key, top_k=8)
        ...
        # Run DuQuant and update lm.model with the possibly modified model returned
        returned_model = duquant(lm, args, dataloader, act_scales, act_shifts, logger=utils.create_logger(Path(args.output_dir)))
        if returned_model is not None:
            lm.model = returned_model

        # Apply FlexQ to remaining (non-sensitive) layers. FlexQ will skip
        # layers marked by DuQuant (we set `_duquant_processed=True`).
        logger.info("Applying FlexQ to remaining layers...")

        # DuQuant sets DuQuant-specific quant params (e.g., 'quant_method','block_size').
        # FlexQ's UniformAffineQuantizer does not accept those keys. Create cleaned
        # FlexQ-compatible quant param dicts and assign them to args before calling flexqllm.
        base_weight_quant_params = {
            "n_bits": args.wbits,
            "per_channel_axes": [0],
            "symmetric": args.symmetric,
            "dynamic_method": args.w_dynamic_method,
        ...
        base_act_quant_params = {
            "n_bits": args.abits if not args.flex_linear_quant else 6,
            "per_channel_axes": [],
            "symmetric": False if not args.flex_linear_quant else args.symmetric,
            "dynamic_method": args.a_dynamic_method,
        }

        # assign cleaned params for all role-specific fields FlexQ expects
        args.weight_quant_params = copy.deepcopy(base_weight_quant_params)
        args.act_quant_params = copy.deepcopy(base_act_quant_params)
        args.q_weight_quant_params = copy.deepcopy(base_weight_quant_params)
        args.k_weight_quant_params = copy.deepcopy(base_weight_quant_params)
        args.v_weight_quant_params = copy.deepcopy(base_weight_quant_params)
        args.o_weight_quant_params = copy.deepcopy(base_weight_quant_params)
        args.gate_weight_quant_params = copy.deepcopy(base_weight_quant_params)
        args.up_weight_quant_params = copy.deepcopy(base_weight_quant_params)
        args.down_weight_quant_params = copy.deepcopy(base_weight_quant_params)

        args.q_act_quant_params = copy.deepcopy(base_act_quant_params)
        args.k_act_quant_params = copy.deepcopy(base_act_quant_params)
        args.v_act_quant_params = copy.deepcopy(base_act_quant_params)
        args.o_act_quant_params = copy.deepcopy(base_act_quant_params)
        args.gate_act_quant_params = copy.deepcopy(base_act_quant_params)
        args.up_act_quant_params = copy.deepcopy(base_act_quant_params)
        args.down_act_quant_params = copy.deepcopy(base_act_quant_params)

        flexqllm(lm, args, utils.create_logger(Path(args.output_dir)))

        # Ensure the entire model uses a consistent dtype before evaluation to avoid
        # matmul dtype mismatches (some DuQuant code toggles float/half during training).
        try:
            target_dtype = args.torch_dtype if isinstance(args.torch_dtype, torch.dtype) else torch.float16
        ...
        # Evaluate
        logger.debug("Args keys before evaluate: %s", sorted(list(vars(args).keys())))
        results = evaluate(lm, args, utils.create_logger(Path(args.output_dir)))
        ppl = results.get("wikitext2", None)
        logger.info("DuQuant Results: PPL=%s", ppl)
    ...
    # Use baseline values (no clipping)
    ...
    run_sweep(args, top_k_layers, percentiles_to_try, zero_scales_to_try, profiles, channel_maxes)
