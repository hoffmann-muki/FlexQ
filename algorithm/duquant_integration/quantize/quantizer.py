...
import numpy as np
import math
from algorithm.duquant_integration.utils import get_rot, exchange_row_col, get_hadamard
from algorithm.duquant_integration.quantize.const import CLIPMAX, CLIPMIN
import random
...
class UniformAffineQuantizer(nn.Module):
    ...
    def _pad_to_block_multiple(self, x: torch.Tensor):
        orig_last = x.shape[-1]
        remainder = orig_last % self.block_size
        if remainder == 0:
            return x, orig_last
        ...
        return torch.cat((x, pad_tensor), dim=-1), orig_last

    def _pad_to_target_last_dim(self, x: torch.Tensor, target_last: int):
        if x.shape[-1] >= target_last:
            return x
        pad_len = target_last - x.shape[-1]
        pad_shape = list(x.shape)
        pad_shape[-1] = pad_len
        pad_tensor = x.new_zeros(*pad_shape)
        return torch.cat((x, pad_tensor), dim=-1)

    def _crop_to_orig_last_dim(self, x: torch.Tensor, orig_last: int):
        if x.shape[-1] == orig_last:
            return x
        return x[..., :orig_last]
    ...
    def rotation(self, weight, max_rotation_step=None, other=None, score_func=None, keep_padded=False):
        if max_rotation_step is None:
            max_rotation_step = self.max_rotation_step
        weight = weight.detach().clone()
        _weight = weight.detach().clone()
        weight, orig_last_weight = self._pad_to_block_multiple(weight)
        padded_hidden = weight.shape[-1]
        exchange_ids = []
        peak_values = []
        if other is not None:
            _other = other.detach().clone()
            other, orig_last_other = self._pad_to_block_multiple(other)
            if other.shape[-1] < padded_hidden:
                other = self._pad_to_target_last_dim(other, padded_hidden)
            elif other.shape[-1] > padded_hidden:
                padded_hidden = other.shape[-1]
                weight = self._pad_to_target_last_dim(weight, padded_hidden)
        else:
            _other = None
            orig_last_other = None
        ...
        weight = torch.matmul(weight.reshape(-1, self.block_size), R_).reshape(-1,padded_hidden)
        if not keep_padded:
            weight = self._crop_to_orig_last_dim(weight, orig_last_weight)
        if other is not None:
            other = torch.matmul(other.reshape(-1, self.block_size), R_).reshape(-1,padded_hidden)
            if not keep_padded:
                other = self._crop_to_orig_last_dim(other, orig_last_other)
    ...
    def online_duquant_cali(self, weight):
        # Pad weight to block_size multiple so all indices stay within bounds
        # Store original shape to restore after calibration
        orig_shape = weight.shape
        weight = weight.detach().clone()
        # Flatten to 2D for calibration: (-1, hidden_dim)
        weight = weight.reshape(-1, weight.shape[-1])
        weight, orig_last = self._pad_to_block_multiple(weight)
        T = {}
        
        self.permutation_list = None
        self.R = None
        # Store padded dimension so init_duquant can match it
        self._calibrated_hidden = weight.shape[-1]
        for i in range(self.permutation_times):
            # rotation expects padded input; keep_padded=True so permutation_zigzag sees padded tensor
            weight, _, R = self.rotation(weight, keep_padded=True)
            if self.R is None:
        ...
        if self.R is None:
            ...
            self.R = torch.cat((self.R, R.unsqueeze(0)), dim=0)
        # Crop back to original dimension and restore original shape
        weight = self._crop_to_orig_last_dim(weight, orig_last)
        # Restore to original shape (with potentially different last dim if cropped)
    ...
    def init_duquant(self, x: torch.Tensor):
        if self.quant_method is None:
            return x
        x, orig_last = self._pad_to_block_multiple(x)
        # If we've calibrated, ensure x is padded to the same dimension used during calibration
        if hasattr(self, '_calibrated_hidden') and x.shape[-1] < self._calibrated_hidden:
            x = self._pad_to_target_last_dim(x, self._calibrated_hidden)
        if self.rotate is None:
            ...
            if self.rotate:
                if not self.init_duquant_params:
                    ...
                    # Ensure x last dim matches permutation indices before capturing x_size
                    if self.permutation_list is not None:
                        perm_dim = self.permutation_list.shape[-1]
                        if x.shape[-1] < perm_dim:
                            x = self._pad_to_target_last_dim(x, perm_dim)
                    # Capture x_size AFTER all padding
                    x_size = x.shape
                    if self.permutation_list is not None:
                        for i in range(len(self.permutation_list)):
                            ...
                            if len(self.permutation_list.shape) == 3:
                                ...
                                # Use index_select on last dimension for N-D tensors
                                x_tmp = x.clone()
                                x[..., perm[0]], x[..., perm[1]] = x_tmp[..., perm[1]], x_tmp[..., perm[0]]
                            else:
                                perm = self.permutation_list[i].to(x.device)
                                # Use index_select on last dimension for N-D tensors
                                x = torch.index_select(x, dim=-1, index=perm)
                    if len(self.R) > 0:
                        x = x.reshape(-1, self.block_size)
                        R = self.R[-1].to(x)
                        x = x.matmul(R).reshape(x_size) 
        ...
        return self._crop_to_orig_last_dim(x, orig_last)
